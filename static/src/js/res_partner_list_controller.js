...
export class ContactsListController extends ListController {
  ...
  async showMergeModal() {
    ...
    if (ids.length < 2) {
      this.notification.add("Please select at least two partners to merge.", {
        title: "Error",
        type: "danger",
        sticky: false,
      });
  ...
  async findSimilarContacts() {
    ...
    try {
      const similarContacts = await this.rpc("/partner/find_similar", {
        partner_id: selectedIds[0],
      });
...
registry.category("views").add("button_in_contacts", {
  model: "res.partner", // Use only for contacts
  ...listView,
  Controller: ContactsListController,
  buttonTemplate: "button_contacts.ListView.Buttons",
});
