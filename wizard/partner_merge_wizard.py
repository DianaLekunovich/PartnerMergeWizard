...
class PartnerMergeWizard(models.TransientModel):
    ...
    main_partner_id = fields.Many2one(
        "res.partner", string="Main Partner", required=True, domain="[('id', 'in', partner_ids)]"
    )
    ...
    def action_merge(self):
        ...
        if len(unique_emails) > 1:
            return {
                ...
                "target": "new",
                "context": {
                    "default_partner_merge_wizard_id": self.id,
...
class PartnerMergeEmailWizard(models.TransientModel):
    ...
    partner_merge_wizard_id = fields.Many2one(
        "partner.merge.wizard", string="Merge Wizard", required=True
    )
    email_line_ids = fields.One2many("partner.merge.email.line", "wizard_id", string="Emails")

    @api.model
    def default_get(self, fields):
        res = super().default_get(fields)
        merge_wizard = self.env["partner.merge.wizard"].browse(
            self._context.get("default_partner_merge_wizard_id")
        )
...
class PartnerMergeEmailLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        "partner.merge.email.wizard", string="Wizard", required=True, ondelete="cascade"
    )
