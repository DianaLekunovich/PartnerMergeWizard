...
4. **Merging Logic:**
   ...
   - Once the user clicks **"Merge"** (or a similar button), the wizard should:
     ...
     - Reassign any related objects (e.g., sales orders, invoices, etc.) from the duplicate partners to the main partner.
       _In a real scenario, you’d identify all models referencing `res.partner` and update their `partner_id` fields. For the purpose of this exercise, you can demonstrate the concept by handling just one or two models—like `sale.order` or `account.move`—or provide an explanation of how you would do so._
